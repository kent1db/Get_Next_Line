...
/*   Updated: 2020/12/01 10:57:19 by qurobert         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
int				ft_last_cpy(char **line, char **buff)
{
	...
	len = ft_strlen(*buff);
	if (!(*line = ft_substr(*buff, 0, len, 0)))
		return (-1);
...
int				get_next_line(int fd, char **line)
{
	...
	while (ret)
	{
		if ((nl = ft_memchr(t[fd], '\n', ft_strlen(t[fd]))) != -1)
		{
			if (!(*line = ft_substr(t[fd], 0, nl, 0)))
				return (-1);
			t[fd] = ft_substr(t[fd], nl + 1, (ft_strlen(t[fd]) - (nl + 1)), 1);
			return (1);
		}
